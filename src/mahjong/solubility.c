 ...
 * structures are modelled on those he used (and the typeinfo structure is
 * identical) but the algorithm is completely different. The new algorithm
 * works by randomly descending the tree of all possible games (i.e all
 * possible sequences of pairs of tiles) and tries to find a path to a
 * solution. If it can't get there it backtracks up the tree trying all the
 * possibilities in a random order. In principle this could be very expensive 
 * computationally, but in practice only a few levels of backtracking should
 * be necessary, although if an unsolvable board is entered then it could
 * fail spectacularly (it will figure out that it is unsolvable, but only
 * after a very long time). This may be unavoidable, but I suspect that given 
 * the contraints of the board (specifically the maximum height) the largest
 * unsolvable board that can be made is not a big problem. Once it has a path 
 * it fills it with pairs of tiles chosen at random. The algorithm is also
 * used to shuffle the tiles. - Callum, 20030819 */
...
struct _typeinfo {
    ...
    int placed;
    int image[2];
    int tile;                   /* A back-reference so we can figure out
...
/* These could be placed on the stack, but they're a little large, so we use
 * the depth variable as a sort of virtual stack pointer. */
gboolean freetiles[MAX_TILES / 2][MAX_TILES];
gboolean filled[MAX_TILES];     /* This is changed incrementally with changes 
                                 * being undone as the program moves back
...
generate_dependencies()
{
    ...
    for (i = 0; i < MAX_TILES; i++)
    {
        ...
        for (j = 0; j < MAX_TILES; j++)
        {
            ...
            /* Nothing we are interested in is outside +/- 1 units on the y
             * axis. */
            if (abs(ty - y) > 1)
                continue;
...
/* This is the private version that additionally does some book keeping for
 * the generate game function. */
static gboolean
...
/* Check all the tiles around a given tile to see if they have become free.
 * This is called after a pair of tiles has been removed. */
static void
...
walk_tree(gint depth)
{
    ...
    if (numfree < 2)
    {                           /* We don't have enough tiles to continue. */
        return FALSE;
    }

    /* Get a list of free tiles remaining. This is a compacted version of
     * freetiles. freetiles should have been constructed by the previous
     * iteration. */
    j = 0;
    for (i = 0; i < MAX_TILES; i++)
    ...
    if (depth == MAX_TILES / 2 - 1)
    {                           /* We have reached the end with precisely two 
                                 * tiles to place. */
    ...
    for (i = 0; i < numfree - 1; i++)
    {
        for (j = i + 1; j < numfree; j++)
        {
            ...
            {
                /* We have found a path to the end. Place a pair of tiles and 
...
shuffle(void)
{
    ...
    /* Figure out which ones are available and which aren't. Also figure out 
     * what depth in the tree we have to start at, i.e. how far through the
     * game we are. */
    for (i = 0; i < MAX_TILES; i++)
    {
        if (tiles[i].visible)
